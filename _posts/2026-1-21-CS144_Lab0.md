...
[Minnow版本](https://github.com/CS144/minnow.git)
...
在*byte_stream_helpers.cc*中实现了接口隔离，让程序更清晰（分开Read和Write行为）

子类Reader和Writer只是提供对ByteStream的读/写操作，不进行任何的状态管理，所有的额外管理都在ByteStream进行实现

>知识点：利用c++对象内存布局，改变编译器对某块内存的解释
...
当一个子类继承一个父类但是*没有添加任何新成员变量的时候*，它和父类之间可以安全的转换
static_assert就是对这个情况的判断，添加了一个断言，如果子类（Reader/Writer）添加了成员变量，就会在编译时候报错
...
最后关键的一点，
此时，this指针是指向ByteStream（因为是ByteStream.reader()），在这里保证安全的前提下，进行强制转换为Reader的指针
修改指针后，此时就只能调用Reader类中的函数
